"""
train_cross_attention_transformer.py

Training script για το 2ο μοντέλο της διπλωματικής: Multimodal
Cross-Attention Transformer (βλ. ΣΕΙΡΑ_ΥΛΟΠΟΙΗΣΗΣ.txt Βήμα 12,
ΔΙΠΛΩΜΑΤΙΚΗ_ΕΡΓΑΣΙΑ.txt "Cross-Attention Transformer Multimodal
Model").

Δομή πανομοιότυπη με το train_hybrid_baseline.py (ίδιο data pipeline,
ίδιο CLI style, heartbeat logging, per-epoch checkpointing, domain-
adversarial training) ώστε τα δύο μοντέλα να είναι άμεσα συγκρίσιμα
με τις ίδιες συνθήκες εκπαίδευσης -- μόνη διαφορά είναι η αρχιτεκτονική
του μοντέλου (Transformer με cross-attention αντί για CNN+MLP) και το
forward signature (eeg, eda, ppg αντί για eeg, physio_features).

Χρήση
-----
Πλήρες dataset (32 subjects, αργό λόγω feature extraction):

    python train_cross_attention_transformer.py --epochs 15

Γρήγορο δοκιμαστικό run σε λίγα subjects (sanity check, λεπτά):

    python train_cross_attention_transformer.py --epochs 10 --num-subjects 8 --trials-per-subject 10
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Training run για το Cross-Attention Transformer."
    )
    ...
    parser.add_argument("--num-heads", type=int, default=None,
                         help="Overrides config.TRANSFORMER_NUM_HEADS if given.")
    parser.add_argument("--self-attn-layers", type=int, default=None,
                         help="Overrides config.TRANSFORMER_SELF_ATTN_LAYERS if given.")
    parser.add_argument("--cross-attn-layers", type=int, default=None,
...
def apply_quick_subset(num_subjects, trials_per_subject):
    ...
    sanity-check training (ίδια λογική με train_hybrid_baseline.py).
    """
...
def augment_eeg(eeg: torch.Tensor, noise_std: float = 0.05, channel_dropout_p: float = 0.10) -> torch.Tensor:
    """
    Data augmentation στο raw EEG, ΜΟΝΟ κατά την εκπαίδευση (ίδια
    λογική/rationale με το train_hybrid_baseline.py -- βοηθά να μην
    "απομνημονεύει" το μοντέλο subject-specific waveform patterns).
    """
...
def build_subject_to_idx(dataset) -> dict:
    """
    Χτίζει mapping subject id -> ακέραιο index, ΜΟΝΟ πάνω στα train
    subjects (βλ. train_hybrid_baseline.py για πλήρες rationale).
    """
...
def run_epoch(
    ...
    Ίδια δομή/rationale με το run_epoch του train_hybrid_baseline.py,
    προσαρμοσμένη στο forward signature του Cross-Attention Transformer
    (eeg, eda, ppg αντί για eeg, physio_features).
    """
    ...
    with context:
        ...
        for batch_idx, batch in enumerate(loader, start=1):
            ...
            out = model(eeg, eda, ppg, grl_lambda=grl_lambda)
            ...
            attn_values.append(out["cross_attention_weight"].detach().cpu().numpy())
    ...
    return {
        ...
        "cross_attn_mean": float(attn_values.mean()),
        "preds": all_preds,
        "labels": all_labels,
    }
...
def main():
    ...
    if args.num_heads is not None:
        config.TRANSFORMER_NUM_HEADS = args.num_heads

    if args.self_attn_layers is not None:
        config.TRANSFORMER_SELF_ATTN_LAYERS = args.self_attn_layers

    if args.cross_attn_layers is not None:
        config.TRANSFORMER_CROSS_ATTN_LAYERS = args.cross_attn_layers
    ...
    # pin_memory μόνο ωφελεί σε CUDA (βλ. train_hybrid_baseline.py rationale).
    config.PIN_MEMORY = (device.type == "cuda")

    print("=" * 80, flush=True)
    print("CROSS-ATTENTION TRANSFORMER -- TRAINING RUN", flush=True)
    print("=" * 80, flush=True)
    ...
    print(f"d_model           : {config.TRANSFORMER_D_MODEL}", flush=True)
    print(f"Num Heads         : {config.TRANSFORMER_NUM_HEADS}", flush=True)
    ...
    # που μπορούσε να οδηγήσει σε ασυνεπή μείωση LR).
    ...
    header = (
        ...
        f"{'Val Loss':>8} | {'Val Acc':>7} | {'Val W.F1':>8} | {'Val T.F1':>8} | {'CrossAttn':>9} | "
    ...
    print(f"Cross-Attn Max (avg): {test_metrics['cross_attn_mean']:.4f}  (1/num_keys={1/24:.4f} == πλήρως ομοιόμορφο)")
