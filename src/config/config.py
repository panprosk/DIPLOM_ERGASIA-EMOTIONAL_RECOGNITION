...
class Config:
    ...
    # Πλάτος "νεκρής ζώνης" γύρω από το VALENCE_THRESHOLD (DEAP SAM
    # valence scale 1-9) μέσα στην οποία τα trials θεωρούνται
    # ασαφή/οριακά (π.χ. valence=5.1 δεν διαφέρει ουσιαστικά σε
    # βιωμένο συναίσθημα από valence=4.9, αλλά παίρνουν αντίθετο
    # binary label) και αποκλείονται εντελώς από το dataset (και τα
    # τρία splits) πριν το windowing. Καθιερωμένη τεχνική στη
    # βιβλιογραφία binary-valence ταξινόμησης πάνω στο DEAP (label-
    # noise reduction γύρω από το decision threshold) — μειώνει τον
    # θόρυβο των ετικετών χωρίς να αλλάζει το ίδιο το πρόβλημα
    # ταξινόμησης. Με MARGIN=1.0 αποκλείονται trials με valence στο
    # [4.0, 6.0]. Θέσε 0.0 για να απενεργοποιηθεί (συμπεριφορά ίδια
    # με πριν).
    #
    # ΕΝΗΜΕΡΩΣΗ: default επανήλθε σε 0.0 (απενεργοποιημένο). Σε 2
    # πλήρη runs με MARGIN=1.0 το Test Trial F1 ήταν σαφώς χειρότερο
    # (0.4377, 0.4233) σε σχέση με το καλύτερο run χωρίς margin
    # (0.5666) — πιθανώς επειδή αφαιρεί 15-50% των trials ανά subject,
    # μειώνοντας πολύ το ήδη μικρό training set με μόνο 22 subjects.
    VALENCE_MARGIN: float = 0.0
